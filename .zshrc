...
function zle-line-init zle-keymap-select {
    case $KEYMAP in
        vicmd)
        PROMPT='%F{yellow}%n@%d%f[%F{red}NOR%f]%F{yellow}$%f '
        ;;
        main|viins)
        PROMPT='%F{yellow}%n@%d%f[%F{blue}INS%f]%F{yellow}$%f '
        ;;
        vivis)
        PROMPT='%F{yellow}%n@%d%f[%F{green}VIS%f]%F{yellow}$%f '
        ;;
    esac
    zle reset-prompt
}
